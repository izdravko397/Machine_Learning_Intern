...
class HalvingGridSearchCV(HalvingBase):
...
class HalvingRandomSearchCV(HalvingBase):
    def __init__(self, estimator, param_distributions, *, scoring='root_mean_squared_error', cv=5, refit=True, 
                 n_candidates=10, factor=3, max_resources='auto', min_resources=100, random_state=None):
        if not hasattr(estimator, 'fit'):
            raise TypeError('Invalid estimator')
        ...
        assert isinstance(n_candidates, int) and n_candidates > 0, 'n_candidates must be int > 0'
...
rnd_search = HalvingRandomSearchCV(full_pipeline, param_distributions=param_distribs, n_candidates=5, cv=3,
    scoring='root_mean_squared_error', min_resources=2000, factor=3, random_state=42)
