...
class StreamHandler(Base_handler):
...
class FileHandler(Base_handler):
    ...
    def print_logger(self, record, exc_info=False):
        ...
        with open(file=self.fname, mode=self.mode, encoding=self.encoding) as f:
            if self.formatter.style == '{':
                f.write(self.formatter.format.format(**record.__dict__) + '\n')
            elif self.formatter.style == '%':
                f.write(self.formatter.format % record.__dict__ + '\n')
            else:
                f.write(Template("$levelname").substitute(**record.__dict__) + '\n')
